...
public class Midi {
	...
	private static File write(Chant c, File out) {
		try {
			Sequence seq = new Sequence(Sequence.PPQ, RESOLUTION, TRACK_NUM);
	 ...
	 */
	private static void writeTrack(Sequence seq, int[] t, int instru, int canal)
	...
	private static MidiEvent noteOn(int nKey, long lTick, int canal)
			throws InvalidMidiDataException {
		return createEvent(ShortMessage.NOTE_ON, nKey, VELOCITY, lTick, canal);
	}

	private static MidiEvent noteOff(int nKey, long lTick, int canal)
			throws InvalidMidiDataException {
		return createEvent(ShortMessage.NOTE_OFF, nKey, 0, lTick, canal);
	}

	private static MidiEvent instrument(int instrument, int canal)
			throws InvalidMidiDataException {
		return createEvent(ShortMessage.PROGRAM_CHANGE, instrument, 0, 0, canal);
	}

	private static MidiEvent createEvent(int nCommand, int nKey, int nVelocity,
	...
	private static int noteToMidi(int note) {
		return CONST1 + 2 * Chant.getNoteBase(note) + CONST2
				* Chant.getOctave(note) - ((isLowerThanFa(note)) ? 0 : 1);
	}
